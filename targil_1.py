...
if __name__ == "__main__":
    ...
    ax1.set_ylabel('morris alpha', color=color)
    ...
    color = 'tab:blue'
    ax2.set_ylabel('fm0', color=color)  # we already handled the x-label with ax1
    ...
    for i in final_counters:
      for t in beta_counters:
        ...
        fm_final_temp_res = (np.mean(z))
        fm_final_res.append(fm_final_temp_res)
        fm_final_temp_var = np.var(z) / len(stream)
        fm_final_var.append(fm_final_temp_var)
        fm_final_temp_error = math.log(1 / delta) / (i * t)
        fm_final_error.append(fm_final_temp_error)
    ...
    x1 = [f'({a},{b})' for a in final_counters for b in beta_counters] #find that online
    x2 = [f'({final_counters},{beta_counters})'] #find that online
    print(x1,x2)
    plt.plot(x, fm_final_res)
    plt.title('Convergence of estimation w.r.t number of alpha copies')
    plt.xlabel('alpha copies')
    plt.ylabel('mean')
    plt.axhline(y=10, linestyle='--', color='red')
    plt.show()

    plt.plot(x, fm_final_var)
    plt.title('Variance w.r.t alpha copies')
    plt.xlabel('alpha copies')
    plt.ylabel('normalized variance')
    plt.show()

    plt.plot(x, fm_final_error)
    plt.title('Error w.r.t alpha copies')
    plt.xlabel('alpha copies')
    plt.ylabel('Error')
    plt.show()
